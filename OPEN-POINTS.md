...
### 36. WORK — `aros-base.pkg` is blocked on one generated header

The boot now loads every package that builds and asks for the libraries in
`aros-base.pkg`: utility, oop, stdc, intuition. That package does not build,
because `linklibs-udis86` does not:

```
arch/all-pc/udis86/libudis86/types.h:216:10: fatal error: 'itab.h' file not found
```

`itab.c` and `itab.h` are generated by
`$(PYTHON) scripts/ud_itab.py docs/x86/optable.xml $(GENDIR)/$(CURDIR)/libudis86`
(`arch/all-pc/udis86/mmakefile.src:26`). The linklib's source list already names
the generated `itab`, and `udis86-includes-gen` already copies `itab.h`, so only
the rule that produces them is missing. It is line 7 of the 21 in
`generated_targets.generated-file-rules.txt` -- the report of Make rules whose
recipe builds a file and which nothing models.

Two questions before writing anything. Whether the other 20 rules are the same
shape, in which case one modelled rule form covers them all rather than a
special case for udis86; and whether the disassembler is reachable at all on the
boot path, because if it is only the alert formatter's optional detail then the
faster route to a graphical boot may be to leave it out of the package and say
so, rather than to build it.
